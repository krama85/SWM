...
class CausalTransitionModel(nn.Module):
    ...
    def __init__(self, embedding_dim_per_object, input_dims, hidden_dim, 
        ...
        num_channels = input_dims[0] # input_dims[0]
        ...
        if self.modular:
            ...
            self.transition_nets = MLPTransition(
                ...
                num_actions=self.action_dim, hidden_dim=self.hidden_dim)
                #num_actions=self.num_objects * self.action_dim, hidden_dim=self.hidden_dim)
        #
        # attn_kwargs = {
        #     'recurrent_type': 'attention',
        #     'layer_kwargs_list': [{'scale_factor': 1, 'lat_channels': 128, 'inter_channels': 128,
        #                            'inter_kernel_size': 3, 'lat_kernel_size': 3, 'groups': 8, 'depth': 1},
        #                           {'scale_factor': 2, 'lat_channels': 128, 'inter_channels': 128,
        #                            'inter_kernel_size': 3, 'lat_kernel_size': 3, 'groups': 8, 'depth': 1}],
        #     'nonlinear_kwargs': {'nonlinear_type': 'elu'}
        # }
        #
        # self.hnet = recurrent(in_channels=256, **attn_kwargs)

        self.encoder = nn.Sequential(OrderedDict(
            obj_extractor=obj_extractor,
            #obj_encoder=self.hnet))
...
class CausalTransitionModelLSTM(nn.Module):
    ...
    def __init__(self, embedding_dim_per_object, input_dims, hidden_dim, action_dim,
                 ...
                 graph=None, modular=False, vae=False, rim = True, scoff = False,
                 multiplier=1):
        ...
        if not self.ignore_action:
            ...
            if self.copy_action:
                action_vec = utils.to_one_hot(
                    action, self.action_dim).repeat(1, 1) # orig -- repeat(1, self.num_objects)
