...
def evaluate(model, loader, *,
    ...
    with torch.no_grad():
        ...
        for batch_idx, data_batch in iterator:
            ...
            obs = observations[0]/255
            next_obs = observations[-1]/255
            ...
            #     next = next_obs[i]
            #     rec = rec_obs[i]
            #     save_image(img, 'output/orig_img' + str(batch_idx) + '_' + str(i) + '.png')
            #     save_image(next, 'output/next_img' + str(batch_idx) + '_' + str(i) + '.png')
            #     save_image(rec, 'output/rec_next_img' + str(batch_idx) + '_' + str(i) + '.png')
            # import pdb; pdb.set_trace()
            if not contrastive:
                rec_obs = model.decoder(next_state)
                rec += image_loss(rec_obs, next_obs).item()
            #     import pdb; pdb.set_trace()
            #
            #     for i in range(0, 10):
            ...
            #         save_image(img, 'output/orig_img' + str(batch_idx) + '_' + str(i) + '.png')
            #         save_image(next, 'output/next_img' + str(batch_idx) + '_' + str(i) + '.png')
            #         save_image(recs, 'output/rec_next_img' + str(batch_idx) + '_' + str(i) + '.png')
            #
            # import pdb; pdb.set_trace()
...
def evaluate_lstm(model, loader, *,
                  device, batch_size, num_steps, silent=False, name="Default", save_folder=None, hidden_dim=600,
                  contrastive=False):
    # topk = [1, 5, 10]
    name = str(name).split('/')[-1]
    topk = [1]
...
def eval_steps(model, steps_array, *,
    ...
    for steps in steps_array:
        ...
        dataset = PathDataset(
            ...
            action_transform=OneHot(action_dim),
...
def eval_steps_lstm(model, steps_array, *,
                    filename, batch_size, device, save_folder, name="Default", action_dim=5, hidden_dim=600,
                    contrastive=False):
    string = ""
    for steps in steps_array:
        print("Loading data...")
